...
**Core Philosophy:** _Dimensional Accuracy over Volume Matching._
...
3.  **Level 3 - Clearances & Contours (Micro):** Add the final details: corner radii (`fillet`, `chamfer`), structural support ribs, and screw head recesses.
    _Note: ALWAYS wrap boolean and fillet results in `_as_solid()` to prevent OpenCASCADE `ShapeList` exceptions._
...
1.  **Run Diff:** Execute the comparison script:
    ...
    ```
    _Ensure the script aligns the solids appropriately (e.g., by bounding box center or specific faces) without artificially offsetting them._
