...
- **Mesh outputs.** `bots/*/meshes/` is auto-generated (not committed to git) — regenerated by `bot.emit()` via ShapeScript to STL. `bots/*/print/` holds slicer-ready STLs for fabricated bodies (committed to git).
