"""CAD emitter — generates per-body STLs for MuJoCo simulation using build123d.

Produces per-body STLs — individual body solids in local frame, referenced by
MuJoCo XML for simulation visuals.  Also writes print-ready STLs for fabricated
bodies to ``print/`` (committed to git, used by slicers).
...
def emit_cad(bot: Bot, output_dir: Path, cad: CadModel) -> None:
    """Generate per-body STL files from pre-built geometry."""
